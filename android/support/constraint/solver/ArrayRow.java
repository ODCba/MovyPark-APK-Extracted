...
public class ArrayRow {
    ...
    public boolean hasKeyVariable() {
        return (this.variable == null || (this.variable.mType != Type.UNRESTRICTED && this.constantValue < 0.0f)) ? false : USE_LINKED_VARIABLES;
    }
            ...
            if (v != null) {
                ...
                if (addedVariable) {
                    if (amount > 0.0f) {
                        ...
                        s = s + " - ";
                        amount *= GroundOverlayOptions.NO_DIMENSION;
                    }
                } else if (amount < 0.0f) {
                    s = s + "- ";
                    amount *= GroundOverlayOptions.NO_DIMENSION;
    ...
    public ArrayRow createRowEquals(SolverVariable variable, int value) {
        if (value < 0) {
            ...
            this.constantValue = (float) value;
            this.variables.put(variable, GroundOverlayOptions.NO_DIMENSION);
        }
        return this;
    }
    ...
    public ArrayRow createRowEquals(SolverVariable variableA, SolverVariable variableB, int margin) {
        ...
        if (inverse) {
            ...
            this.variables.put(variableB, GroundOverlayOptions.NO_DIMENSION);
        } else {
            this.variables.put(variableA, GroundOverlayOptions.NO_DIMENSION);
    ...
    public ArrayRow createRowGreaterThan(SolverVariable variableA, SolverVariable variableB, SolverVariable slack, int margin) {
        ...
        if (inverse) {
            this.variables.put(variableA, 1.0f);
            this.variables.put(variableB, GroundOverlayOptions.NO_DIMENSION);
            this.variables.put(slack, GroundOverlayOptions.NO_DIMENSION);
        } else {
            this.variables.put(variableA, GroundOverlayOptions.NO_DIMENSION);
            this.variables.put(variableB, 1.0f);
    ...
    public ArrayRow createRowEqualDimension(float currentWeight, float totalWeights, float nextWeight, SolverVariable variableStartA, int marginStartA, SolverVariable variableEndA, int marginEndA, SolverVariable variableStartB, int marginStartB, SolverVariable variableEndB, int marginEndB) {
        if (totalWeights == 0.0f || currentWeight == nextWeight) {
            ...
            this.variables.put(variableEndA, GroundOverlayOptions.NO_DIMENSION);
            this.variables.put(variableEndB, 1.0f);
            this.variables.put(variableStartB, GroundOverlayOptions.NO_DIMENSION);
        } else {
    ...
    public ArrayRow createRowCentering(SolverVariable variableA, SolverVariable variableB, int marginA, float bias, SolverVariable variableC, SolverVariable variableD, int marginB, boolean withError) {
        if (variableB == variableC) {
            ...
            this.variables.put(variableA, 1.0f);
            this.variables.put(variableB, GroundOverlayOptions.NO_DIMENSION);
            this.variables.put(variableC, GroundOverlayOptions.NO_DIMENSION);
            this.variables.put(variableD, 1.0f);
            ...
            this.variables.put(variableB, (1.0f - bias) * GroundOverlayOptions.NO_DIMENSION);
            this.variables.put(variableC, GroundOverlayOptions.NO_DIMENSION * bias);
    ...
    public ArrayRow addError(SolverVariable error1, SolverVariable error2) {
        ...
        this.variables.put(error2, GroundOverlayOptions.NO_DIMENSION);
        return this;
    }
    ...
    public void ensurePositiveConstant() {
        if (this.constantValue < 0.0f) {
            this.constantValue *= GroundOverlayOptions.NO_DIMENSION;
    ...
    public void pivot(SolverVariable v) {
        if (this.variable != null) {
            this.variables.put(this.variable, GroundOverlayOptions.NO_DIMENSION);
            this.variable = null;
        }
        float amount = this.variables.remove(v) * GroundOverlayOptions.NO_DIMENSION;
        this.variable = v;

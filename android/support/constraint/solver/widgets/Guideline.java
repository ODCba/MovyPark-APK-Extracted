...
public class Guideline extends ConstraintWidget {
    ...
    protected float mRelativePercent = GroundOverlayOptions.NO_DIMENSION;
    ...
    public int getRelativeBehaviour() {
        if (this.mRelativePercent != GroundOverlayOptions.NO_DIMENSION) {
            return 0;
        }
    ...
    public void setGuidePercent(float value) {
        if (value > GroundOverlayOptions.NO_DIMENSION) {
    ...
    public void setGuideBegin(int value) {
        if (value > -1) {
            this.mRelativePercent = GroundOverlayOptions.NO_DIMENSION;
    ...
    public void addToSolver(LinearSystem system, int group) {
        ...
        if (parent != null) {
            ...
            if (this.mRelativeBegin != -1) {
                system.addConstraint(EquationCreation.createRowEquals(system, system.createObjectVariable(this.mAnchor), system.createObjectVariable(begin), this.mRelativeBegin, false));
            } else if (this.mRelativeEnd != -1) {
                system.addConstraint(EquationCreation.createRowEquals(system, system.createObjectVariable(this.mAnchor), system.createObjectVariable(end), -this.mRelativeEnd, false));
            } else if (this.mRelativePercent != GroundOverlayOptions.NO_DIMENSION) {
                system.addConstraint(EquationCreation.createRowDimensionPercent(system, system.createObjectVariable(this.mAnchor), system.createObjectVariable(begin), system.createObjectVariable(end), this.mRelativePercent, this.mIsPositionRelaxed));
                if (this.mMinimumPosition <= 0) {
                }
            }
        }
    }
